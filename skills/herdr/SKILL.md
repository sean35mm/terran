...
<!-- Derived from herdrdev/herdr v0.8.2, skills/herdr/SKILL.md. The Terran privacy/safety overlay below is a local modification. Refresh official content with: herdr --skill -->

## Local privacy and safety overlay

Existing panes and agents, and their output, are private. Inspect their content only when the user identified or authorized that target, or when it was created for the current task; do not search unrelated panes. Never read, reproduce, or persist secrets, and redact sensitive output. Topology and status metadata may be inspected when needed to identify the authorized target.

The official temporary-Markdown fallback writes a complete agent response to a file. Avoid this fallback for sensitive output. When it is authorized and needed, use a securely created user-private temporary directory, read only the expected file, and delete the file and directory immediately after use. Never persist secrets.
